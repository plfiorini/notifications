...
def find_customer_id(session, notification_text):
    ...
    if len(labels) > 1:
        ...
        customer = (
            ...
            .filter(Customer.notification_label.ilike(labels[0]))
            .first()
        )
        id_customer = customer.id
        logging.debug(
            f"Cliente '{customer.name}' trovato con label '{labels[0]}' per la notifica '{notification_text}'"
        )
    else:
...
def process_notification(notification_text, session):
    ...
    if id_customer is not None:
        ...
        if query.count() == 0:
            session.add(
                NotificationCounter(
                    id_customer=id_customer, day=datetime.date.today(), num=1
                )
            )
        else:
