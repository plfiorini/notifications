...
from server import Base, Customer, Notification, NotificationCounter, process_notification
...
class TestNotifications(unittest.TestCase):
    ...
    def test_valid_notification(self):
        ...
        notification_counter = self.session.query(NotificationCounter).filter_by(id_customer=notification.customer.id, day=datetime.date.today()).one()
    ...
    def test_no_customer(self):
        ...
        process_notification(notification_text, self.session)
        notifications = self.session.query(Notification).filter(Notification.body == notification_text)
        self.assertEqual(notifications.count(), 1)
        notification = notifications.one()
